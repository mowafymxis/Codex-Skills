...
import type { Config, Competitor } from "./types.js";
...
function normalizeCompetitor(value: unknown): Competitor {
  if (!value || typeof value !== "object") throw new Error("Each competitor must be an object");
  const raw = value as Record<string, unknown>;
  if (typeof raw.name !== "string" || !raw.name.trim())
    throw new Error("Competitor name is required");
  if (typeof raw.url !== "string") throw new Error(`Competitor ${raw.name} requires a URL`);
  try {
    new URL(raw.url);
  } catch {
    throw new Error(`Competitor ${raw.name} has an invalid URL`);
  }
  const allowed = ["direct", "indirect", "substitute", "open-source", "benchmark"] as const;
  const type = allowed.includes(raw.type as (typeof allowed)[number])
    ? (raw.type as Competitor["type"])
    : "direct";
  return {
...
export async function loadConfig(
  ...
  if (file) raw = JSON.parse(await readFile(file, "utf8")) as Record<string, unknown>;
  const merged = { ...defaults, ...raw, ...overrides } as Config;
  merged.competitors = ((overrides.competitors ?? raw.competitors ?? []) as unknown[]).map(
    normalizeCompetitor,
  );
  if (
    !Number.isInteger(merged.maxPagesPerCompetitor) ||
  ...
  merged.outputDirectory = path.resolve(
