...
export function buildAnalysis(config, appFeatures, competitorFeatures, sources) {
    const normalized = normalizeFeatures(appFeatures, competitorFeatures, config.aliasDictionary);
    const featureGaps = normalized.map((feature) => {
        const competitors = feature.competitorAvailability;
        const appStatus = String(feature.appStatus);
        const missing = appStatus === "missing";
        const inputs = defaultScore(missing, competitors.length, String(feature.confidence), String(feature.category));
        if (missing &&
        ...
        const appEvidence = feature.evidence.filter((x) => x.type === "code");
        const competitiveStatus = missing && competitors.length
        ...
        if (!missing && !competitors.length)
            recommendation = "Differentiator to Protect";
        else if (!missing && ["partial", "prototype", "mentioned-only"].includes(appStatus))
            recommendation = "Improve Existing";
        else if (!missing)
            recommendation = "Already Strong";
        return {
            ...
            evidence: appEvidence.length ? appEvidence : feature.evidence,
            ...
            risks: inputs.evidenceStrength <= 2
    ...
    return {
        ...
        growthRetentionAnalysis: {
            status: config.includeGrowthAnalysis || config.includeRetentionAnalysis
                ? "evidence-limited"
                : "not-requested",
            recommendations: [],
        },
        ...
        positioning: {
            strongestVerifiedAngle: featureGaps.find((x) => x.competitiveStatus === "app advantage")?.feature ??
        ...
        warnings: sources.length === 0 && config.competitors.length
