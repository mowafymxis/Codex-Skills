...
export function extractFeatures(sources) {
    const features = [];
    for (const source of sources.filter((x) => ["success", "cached", "manual"].includes(x.fetchStatus))) {
        for (const [name, category, pattern] of FEATURE_RULES) {
            const match = source.text.match(pattern);
            if (!match)
                continue;
            const evidenceType = source.sourceType === "pricing"
                ? "pricing-confirmed"
                : source.sourceType === "documentation"
                    ...
                    : source.isOfficial
                        ...
                        : "third-party-review";
            const confidence = source.reliability === "high" ? "high" : source.reliability === "medium" ? "medium" : "low";
            features.push({
    ...
    for (const feature of features) {
        ...
        if (!existing || confidenceRank(feature.confidence) > confidenceRank(existing.confidence))
            ...
            existing.sources = [...new Set([...existing.sources, ...feature.sources])];
    }
    return [...unique.values()].sort((a, b) => a.id.localeCompare(b.id));
}
const confidenceRank = (value) => ({ unknown: 0, low: 1, medium: 2, high: 3 })[value] ?? 0;
