...
export async function diffReports(previousDir, currentDir, out) {
    const previous = JSON.parse(await readFile(path.join(previousDir, "feature-gap-analysis.json"), "utf8"));
    const current = JSON.parse(await readFile(path.join(currentDir, "feature-gap-analysis.json"), "utf8"));
    const changes = [];
    compareById(previous.appFeatures, current.appFeatures, (oldValue, newValue) => {
        if (!oldValue && newValue)
            return `New app feature: ${newValue.name}`;
        if (oldValue && !newValue)
            return `App feature removed from evidence: ${oldValue.name}`;
        if (oldValue &&
            newValue &&
    ...
    compareById(previous.competitorFeatures, current.competitorFeatures, (oldValue, newValue) => !oldValue && newValue
