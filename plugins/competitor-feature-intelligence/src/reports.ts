...
export async function generateReports(analysis: Analysis, out: string): Promise<void> {
  const jsonFiles: Record<string, unknown> = {
    ...
    "analysis-metadata.json": {
      ...
      sourcesRetrieved: analysis.sources.filter((x) =>
  ...
  await atomicWrite(
    path.join(out, "positioning-recommendations.md"),
    `# Positioning Recommendations\n\nStrongest verified angle: ${String(analysis.positioning.strongestVerifiedAngle)}\n\nSupported claims: ${(analysis.positioning.supportedClaims as string[]).join(", ") || "None yet; gather stronger evidence."}\n\nAvoid definitive competitor-absence and compliance claims.\n`,
  );
  await atomicWrite(
    path.join(out, "technical-feasibility.md"),
...
function mainReport(a: Analysis): string {
  const partial = a.warnings.length ? `> **Partial analysis:** ${a.warnings.join(" ")}\n\n` : "";
  const table = a.appFeatures
    .map(
      (x) =>
        `| ${x.name} | ${x.category} | ${x.status} | ${x.maturityScore} | ${x.confidence} | ${x.evidence
          .map((e) => e.path ?? e.url)
          .filter(Boolean)
...
function roadmapReport(a: Analysis): string {
...
function doNotBuildReport(a: Analysis): string {
...
function matrixCsv(a: Analysis): string {
  const headers = [
    "Feature ID",
    "Feature",
    "Category",
...
export async function validateReportDirectory(
  ...
  for (const file of REPORT_FILES) {
    try {
      if (!(await stat(path.join(directory, file))).isFile()) missing.push(file);
    } catch {
      missing.push(file);
    }
  }
  if (missing.length) throw new Error(`Missing report files: ${missing.join(", ")}`);
  const schema = JSON.parse(await readFile(schemaPath, "utf8"));
  const ajv = new Ajv2020({ allErrors: true, strict: true });
  addFormats(ajv);
  const validate = ajv.compile(schema);
  const analysis = JSON.parse(
    await readFile(path.join(directory, "feature-gap-analysis.json"), "utf8"),
  );
...
export function issueDrafts(gaps: FeatureGap[], labels: string[]): string {
...
const normalizeLabel = (x: string): string => x.toLowerCase().replace(/[ _-]/g, "");
