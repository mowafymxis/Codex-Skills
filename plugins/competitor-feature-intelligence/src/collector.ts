...
import type { Competitor, Config, SourceRecord } from "./types.js";
import { canonicalizeUrl, hash, redact, sleep, stableId } from "./util.js";

function domainAllowed(url: URL, config: Config, competitor: Competitor): boolean {
  const host = url.hostname.toLowerCase();
  const official = new URL(competitor.url).hostname.toLowerCase();
  if (config.blockedDomains.some((x) => host === x || host.endsWith(`.${x}`))) return false;
  if (
...
function htmlText(html: string): { title: string; text: string; links: string[] } {
  const title =
    html
...
async function robotsAllows(origin: string, pathname: string, timeoutMs: number): Promise<boolean> {
  try {
...
export async function collectSources(
  ...
  if (config.offline) return records;
  for (const competitor of config.competitors) {
    ...
    while (queue.length && seen.size < config.maxPagesPerCompetitor) {
      ...
      if (!domainAllowed(parsed, config, competitor)) {
        records.push(
          failureRecord(
            ...
            canonical,
            "blocked",
            "Domain is outside configured competitor scope.",
          ),
        );
        continue;
      }
      ...
      try {
        const cached = JSON.parse(await readFile(cacheFile, "utf8")) as SourceRecord;
      ...
      try {
        ...
        if (contentSeen.has(contentHash)) continue;
        ...
        const record: SourceRecord = {
          ...
          relevantSnippets: [],
          reliability:
            inferSourceType(finalUrl) === "pricing" || inferSourceType(finalUrl) === "documentation"
              ? "high"
              : "medium",
          notes: [],
        };
        ...
        if (item.depth < config.maxCrawlDepth)
          for (const link of parsedHtml.links) {
            try {
              ...
              if (
                domainAllowed(absolute, config, competitor) &&
...
function inferSourceType(url: string): string {
  if (/pricing|plans/i.test(url)) return "pricing";
  if (/docs|help|guide|api/i.test(url)) return "documentation";
  if (/change|release|blog/i.test(url)) return "changelog";
  if (/security|privacy|terms|trust/i.test(url)) return "trust";
  return "product";
}

export async function ingestManualSources(file: string): Promise<SourceRecord[]> {
  const parsed = JSON.parse(await readFile(file, "utf8")) as unknown;
  if (!Array.isArray(parsed)) throw new Error("Manual source file must contain an array");
  return parsed.map((raw, index) => {
    if (!raw || typeof raw !== "object") throw new Error(`Manual source ${index} is invalid`);
    const x = raw as Partial<SourceRecord>;
    ...
    return {
      ...
      fetchStatus:
        x.fetchStatus === "failed" || x.fetchStatus === "blocked" ? x.fetchStatus : "manual",
      httpStatus: null,
      text,
