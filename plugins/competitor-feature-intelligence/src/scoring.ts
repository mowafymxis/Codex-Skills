...
export function scorePriority(input: ScoreInputs): {
  score: number;
  label: string;
  rationale: string;
} {
      ...
      : score >= 2.8
        ...
        : score >= 2.1
          ...
          : score >= 1.4
            ? "Later"
            : "Ignore";
  if (input.securityBlocker) label = "Build Now";
  else if (input.damagesSimplicity) label = "Do Not Build Yet";
  else if ((input.difficulty >= 4 || input.operationalCost >= 4) && input.evidenceStrength <= 2)
    label = "Validate First";
  else if (input.difficulty >= 4 && input.importance <= 2) label = score < 1.5 ? "Ignore" : "Later";
  return {
    score,
    label,
