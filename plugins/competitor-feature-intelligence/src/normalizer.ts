...
export function canonicalName(name: string, aliases: Record<string, string> = {}): string {
  ...
  return (
    {
      ...SAFE_ALIASES,
      ...Object.fromEntries(
        Object.entries(aliases).map(([k, v]) => [k.toLowerCase(), v.toLowerCase()]),
      ),
...
export function normalizeFeatures(
  ...
  const catalog = new Map<string, { app: AppFeature[]; competitors: CompetitorFeature[] }>();
  ...
  return [...catalog.entries()]
    ...
    .map(([name, group]) => ({
      ...
      exclusionRules: DISTINCT.filter((pair) => pair.includes(name)).flatMap((pair) =>
        pair.filter((x) => x !== name),
      ),
      relatedButDistinct: DISTINCT.filter((pair) => pair.includes(name)).flatMap((pair) =>
        pair.filter((x) => x !== name),
      ),
      ...
      evidence: [
        ...
        ...group.competitors.flatMap((x) =>
          x.sources.map((url) => ({ type: "source", url, summary: `Supports ${x.name}.` })),
        ),
      ],
